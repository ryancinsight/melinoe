    ...
    F: Fn(usize, WriterShard<'_, 'brand, T>) -> R + Sync,
{
    ...
    let mut chunks = WriterShard::new(cells).chunks(chunk);
    ...
    if let Some(executor) = registered_parallel_executor() {
            ...
            F: Fn(usize, WriterShard<'_, 'brand, T>) -> R + Sync,
        {
            ...
            let result =
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| (ctx.f)(start, shard)));
    ...
    if num_chunks == 1 {
        let shard = chunks.next().expect("num_chunks is 1");
        return alloc::vec![f(0, shard)];
    }
    ...
    std::thread::scope(|scope| {
        ...
        for _ in 0..(num_chunks - 1) {
            let shard = chunks.next().expect("num_chunks > 1");
            let shard_start = start;
            start += shard.len();
            handles.push(scope.spawn(move || f(shard_start, shard)));
        }

        let last_shard = chunks.next().expect("last shard exists");
        let last_start = start;
        let last_res = f(last_start, last_shard);
    ...
    F: Fn(usize, &[T]) -> R + Sync,
{
    ...
    let num_chunks = if slice.is_empty() {
        0
    } else {
        1 + (slice.len() - 1) / chunk
    };
    ...
    if let Some(executor) = registered_parallel_executor() {
        ...
        unsafe fn task_wrapper<T, R, F>(index: usize, data: *mut ())
        where
            T: Sync,
            R: Send,
            F: Fn(usize, &[T]) -> R + Sync,
        {
            let ctx = unsafe { &*(data as *const Context<'_, T, R, F>) };
            let start = index * ctx.chunk_size;
            let end = (start + ctx.chunk_size).min(ctx.slice_len);

            let chunk_ref =
                unsafe { core::slice::from_raw_parts(ctx.slice_ptr.add(start), end - start) };

            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                (ctx.f)(start, chunk_ref)
            }));
        ...
        unsafe {
            executor(
                num_chunks,
                task_wrapper::<T, R, F>,
                &mut ctx as *mut Context<'_, T, R, F> as *mut (),
            );
        }
    ...
    std::thread::scope(|scope| {
        ...
        for index in 0..(num_chunks - 1) {
            ...
            let end = start + chunk;
            let shard = &slice[start..end];
            handles.push(scope.spawn(move || f(start, shard)));
        }

        let last_start = (num_chunks - 1) * chunk;
        let last_shard = &slice[last_start..];
        let last_res = f(last_start, last_shard);
